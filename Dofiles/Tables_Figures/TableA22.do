...
// House quality
	...
	su p_pay_ease, d
	g p_pay_ease_hi = p_pay_ease>=`r(p50)'
	replace p_pay_ease_hi=. if p_pay_ease==.
...
***********
		...
		eststo clear
			...
			* pay_ease redefine + include time FE? (w/ and without)
			
			g combine_pay_ease = p_pay_ease if tmt==3
			replace combine_pay_ease = p_pay_ease if inlist(tmt,1,2)
			
			g t_cliXcombine_pay_ease = t_cli*combine_pay_ease
			g t_lXcombine_pay_ease = t_l*combine_pay_ease
			
			
		// Predicted pay ease function predicting visits/payment in C and L
		eststo: reg visit_post_carto combine_pay_ease t_cliXcombine_pay_ease t_cli walls_final roof_final ravine_final i.house i.stratum if inlist(tmt,1,3),cluster(a7)
			estadd scalar Clusters = `e(N_clust)'
			sum visit_post_carto if t_c==1 & combine_pay_ease!=. & walls_final!=. & roof_final!=. & ravine_final!=.
			estadd local Mean=abs(round(`r(mean)',.001))
			estadd scalar Observations = `e(N)'
		eststo: reg taxes_paid combine_pay_ease t_cliXcombine_pay_ease t_cli walls_final roof_final ravine_final i.house i.stratum if inlist(tmt,1,3),cluster(a7)
			estadd scalar Clusters = `e(N_clust)'
			sum taxes_paid if t_c==1 & combine_pay_ease!=. & walls_final!=. & roof_final!=. & ravine_final!=.
			estadd local Mean=abs(round(`r(mean)',.001))
			estadd scalar Observations = `e(N)'
		eststo: reg visit_post_carto combine_pay_ease t_cliXcombine_pay_ease t_cli walls_final roof_final ravine_final i.house i.stratum if inlist(tmt,2,3),cluster(a7)
			estadd scalar Clusters = `e(N_clust)'
			sum visit_post_carto if t_l==1 & combine_pay_ease!=. & walls_final!=. & roof_final!=. & ravine_final!=.
			estadd local Mean=abs(round(`r(mean)',.001))
			estadd scalar Observations = `e(N)'
		eststo: reg taxes_paid combine_pay_ease t_cliXcombine_pay_ease t_cli walls_final roof_final ravine_final i.house i.stratum if inlist(tmt,2,3),cluster(a7)
			estadd scalar Clusters = `e(N_clust)'
			sum taxes_paid if t_l==1 & combine_pay_ease!=. & walls_final!=. & roof_final!=. & ravine_final!=.
			estadd local Mean=abs(round(`r(mean)',.001))
			estadd scalar Observations = `e(N)'
			
		esttab using "${reploutdir}/chiefs_info_payease_interact.tex", ///
		replace label b(%9.3f) se(%9.3f) ///
		keep (combine_pay_ease t_cliXcombine_pay_ease t_cli t_lXcombine_pay_ease t_l walls_final roof_final ravine_final) ///
		order(combine_pay_ease t_cliXcombine_pay_ease t_cli t_lXcombine_pay_ease t_l walls_final roof_final ravine_final) ///
		scalar(Clusters Mean) sfmt(0 0 3) ///
		nomtitles ///
		mgroups("Visited" "Compliance" "Visited" "Compliance" "Visited" "Compliance", pattern(1 1 1 1) prefix(\multicolumn{@span}{c}{) suffix(}) span) ///
...
***********
		...
		eststo clear
			...
			* willingness redefine + include time FE? (w/ and without)
			
			g combine_willingness = p_willingness if tmt==3
			replace combine_willingness = p_willingness if inlist(tmt,1,2)
			
			g t_cliXcombine_willingness = t_cli*combine_willingness
			g t_lXcombine_willingness = t_l*combine_willingness
			
		// Predicted pay ease function predicting visits/payment in C and L
		eststo: reg visit_post_carto combine_willingness t_cliXcombine_willingness t_cli walls_final roof_final ravine_final i.house i.stratum if inlist(tmt,1,3),cluster(a7)
			estadd scalar Clusters = `e(N_clust)'
			sum visit_post_carto if t_c==1 & combine_willingness!=. & walls_final!=. & roof_final!=. & ravine_final!=.
			estadd local Mean=abs(round(`r(mean)',.001))
			estadd scalar Observations = `e(N)'
		eststo: reg taxes_paid combine_willingness t_cliXcombine_willingness t_cli walls_final roof_final ravine_final i.house i.stratum if inlist(tmt,1,3),cluster(a7)
			estadd scalar Clusters = `e(N_clust)'
			sum taxes_paid if t_c==1 & combine_willingness!=. & walls_final!=. & roof_final!=. & ravine_final!=.
			estadd local Mean=abs(round(`r(mean)',.001))
			estadd scalar Observations = `e(N)'
		eststo: reg visit_post_carto combine_willingness t_cliXcombine_willingness t_cli walls_final roof_final ravine_final i.house i.stratum if inlist(tmt,2,3),cluster(a7)
			estadd scalar Clusters = `e(N_clust)'
			sum visit_post_carto if t_l==1 & combine_willingness!=. & walls_final!=. & roof_final!=. & ravine_final!=.
			estadd local Mean=abs(round(`r(mean)',.001))
			estadd scalar Observations = `e(N)'
		eststo: reg taxes_paid combine_willingness t_cliXcombine_willingness t_cli walls_final roof_final ravine_final i.house i.stratum if inlist(tmt,2,3),cluster(a7)
			estadd scalar Clusters = `e(N_clust)'
			sum taxes_paid if t_l==1 & combine_willingness!=. & walls_final!=. & roof_final!=. & ravine_final!=.
			estadd local Mean=abs(round(`r(mean)',.001))
			estadd scalar Observations = `e(N)'
			
		esttab using "${reploutdir}/chiefs_info_wtp_interact.tex", ///
		replace label b(%9.3f) se(%9.3f) ///
		keep (combine_willingness t_cliXcombine_willingness t_cli t_lXcombine_willingness t_l walls_final roof_final ravine_final) ///
		order(combine_willingness t_cliXcombine_willingness t_cli t_lXcombine_willingness t_l walls_final roof_final ravine_final) ///
		scalar(Clusters Mean  Clusters2 Mean2) sfmt(0 0 3 0 3) ///
		nomtitles ///
		mgroups("Visited" "Compliance" "Visited" "Compliance" "Visited" "Compliance", pattern(1 1 1 1 1 1) prefix(\multicolumn{@span}{c}{) suffix(}) span) ///
		indicate("House FE = *house*""Stratum FE = *stratum*") ///
		star(* 0.10 ** 0.05 *** 0.001) ///
		nogaps nonotes compress
