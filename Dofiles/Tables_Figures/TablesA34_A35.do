...
**********************
	...
	* Define interactions
	...
	g t_lXhq_y = t_l*hq_y
	g t_lXinc_mo_avg = t_l*inc_mo_avg
	g t_lXecon_ind = t_l*econ_ind
	
	g house_reg = 0 if house==1
	replace house_reg = 1 if house==2
	
	g t_lXhr = t_l*house_reg
	g hr = house_reg
	
	eststo clear
	label var t_l "Local"
	label var t_lXhr "Local X High-Value Band"
	label var t_lXhq_y "Local X House Quality"
	label var t_lXinc_mo_avg "Local X Avg. Mon. Income"
	label var t_lXecon_ind "Local X Liquidity Index"
	label var hr "High-Value Band"
	label var hq_y "House Quality"
	label var inc_mo_avg "Avg. Mon. Income"
	label var econ_ind "Liquidity Index"
	label var a7_hq_y "House Quality Nbhd. Mean"
	label var a7_inc_mo_avg "Avg. Mon. Income Nbhd. Mean"
	label var a7_econ_ind "Liquidity Index Nbhd. Mean"
...
*************
	...
	* House Type - Compliance
	eststo: reg taxes_paid t_l t_lXhr hr i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2), cl(a7)
	...
	* House Type - Revenues
	eststo: reg taxes_paid_amt t_l t_lXhr hr i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2), cl(a7)
		su taxes_paid_amt if t_c==1 & time_FE_tdm_2mo_CvL!=. & house==1
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
		estadd scalar Clusters = `e(N_clust)'
		
	esttab using "${reploutdir}/incidence_interactions_house.tex", ///
	replace label b(%9.3f) se(%9.3f) ///
	keep (t_l t_lXhr hr) ///
	order(t_l t_lXhr hr) ///
	scalar(Clusters Mean) sfmt(0 3) ///
	nomtitles ///
	mgroups("Compliance" "Revenues", pattern(1 1) prefix(\multicolumn{@span}{c}{) suffix(}) span) ///
...
*************
	...
	* House Quality - Compliance
	...
	cap drop hq_y_split
	g hq_y_split = 0 if hq_y<0
	replace hq_y_split = 1 if hq_y>0 & hq_y<.
	
	g inc_mo_avg_split = 0 if inlist(tmt,1,2) & time_FE_tdm_2mo_CvL!=. & inc_mo_avg!=.
	sum inc_mo_avg if inlist(tmt,1,2) & time_FE_tdm_2mo_CvL!=.,d
	replace inc_mo_avg_split = 1 if hq_y>`r(p50)' & inlist(tmt,1,2) & time_FE_tdm_2mo_CvL!=. & inc_mo_avg!=.
	
	g econ_ind_split = 0 if inlist(tmt,1,2) & time_FE_tdm_2mo_CvL!=. & econ_ind!=.
	sum econ_ind if inlist(tmt,1,2) & time_FE_tdm_2mo_CvL!=.,d
	replace econ_ind_split = 1 if hq_y>`r(p50)' & inlist(tmt,1,2) & time_FE_tdm_2mo_CvL!=. & econ_ind!=.
	
	cap drop t_lXhq_y_split
	g t_lXhq_y_split = t_l*hq_y_split
	g t_lXinc_mo_avg_split = t_l*inc_mo_avg_split
	g t_lXecon_ind_split = t_l*econ_ind_split
	
	cap drop hetmargin
	cap drop t_lXhetmargin
	g hetmargin = hq_y_split
	g t_lXhetmargin = t_lXhq_y_split
	eststo: reg taxes_paid t_l t_lXhetmargin hetmargin i.house i.stratum i.time_FE_tdm_2mo_CvL a7_hq_y if inlist(tmt,1,2), cl(a7)
		su taxes_paid if t_c==1 & time_FE_tdm_2mo_CvL!=. & hq_y!=. & hetmargin==0
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
		estadd scalar Clusters = `e(N_clust)'
	* Income - Compliance
	cap drop hetmargin
	cap drop t_lXhetmargin
	g hetmargin = inc_mo_avg_split
	g t_lXhetmargin = t_lXinc_mo_avg_split
	eststo: reg taxes_paid t_l t_lXhetmargin hetmargin i.house i.stratum i.time_FE_tdm_2mo_CvL a7_inc_mo_avg if inlist(tmt,1,2), cl(a7)
		su taxes_paid if t_c==1 & time_FE_tdm_2mo_CvL!=. & inc_mo_avg!=. & hetmargin==0
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
		estadd scalar Clusters = `e(N_clust)'
	...
	* Liquidity - Compliance
	cap drop hetmargin
	cap drop t_lXhetmargin
	g hetmargin = econ_ind_split
	g t_lXhetmargin = t_lXecon_ind_split
	eststo: reg taxes_paid t_l t_lXhetmargin hetmargin i.house i.stratum i.time_FE_tdm_2mo_CvL a7_econ_ind if inlist(tmt,1,2), cl(a7)
		su taxes_paid if t_c==1 & time_FE_tdm_2mo_CvL!=. & econ_ind!=. & hetmargin==0
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
		estadd scalar Clusters = `e(N_clust)'
		
	lab var t_l "Local"
	lab var t_lXhetmargin "Local X Above Median"
	lab var hetmargin "Above Median"

	esttab using "${reploutdir}/incidence_interactions_compl.tex", ///
	replace label b(%9.3f) se(%9.3f) ///
	keep (t_l t_lXhetmargin hetmargin) ///
	order(t_l t_lXhetmargin hetmargin) ///
	scalar(Clusters Mean) sfmt(0 3) ///
	nomtitles ///
	mgroups("HQ Compliance" "Inc Compliance" "Liq Compliance", pattern(1 1 1) prefix(\multicolumn{@span}{c}{) suffix(}) span) ///
	...
	* House Quality - Revenues
	cap drop hetmargin
	cap drop t_lXhetmargin
	g hetmargin = hq_y_split
	g t_lXhetmargin = t_lXhq_y_split
	eststo: reg taxes_paid_amt t_l t_lXhetmargin hetmargin i.house i.stratum i.time_FE_tdm_2mo_CvL a7_hq_y if inlist(tmt,1,2), cl(a7)
		su taxes_paid_amt if t_c==1 & time_FE_tdm_2mo_CvL!=. & hq_y!=. & hetmargin==0
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
		estadd scalar Clusters = `e(N_clust)'
	* Income - Revenues
	...
	g hetmargin = inc_mo_avg_split
	g t_lXhetmargin = t_lXinc_mo_avg_split
	eststo: reg taxes_paid_amt t_l t_lXhetmargin hetmargin i.house i.stratum i.time_FE_tdm_2mo_CvL a7_inc_mo_avg if inlist(tmt,1,2), cl(a7)
		su taxes_paid_amt if t_c==1 & time_FE_tdm_2mo_CvL!=. & inc_mo_avg!=. & hetmargin==0
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
		estadd scalar Clusters = `e(N_clust)'
	...
	* Liquidity - Revenues
	...
	g hetmargin = econ_ind_split
	g t_lXhetmargin = t_lXecon_ind_split
	eststo: reg taxes_paid_amt t_l t_lXhetmargin hetmargin i.house i.stratum i.time_FE_tdm_2mo_CvL a7_econ_ind if inlist(tmt,1,2), cl(a7)
		su taxes_paid_amt if t_c==1 & time_FE_tdm_2mo_CvL!=. & econ_ind!=. & hetmargin==0
		estadd local Mean=abs(round(`r(mean)',.001))
		estadd scalar Observations = `e(N)'
		estadd scalar Clusters = `e(N_clust)'

	esttab using "${reploutdir}/incidence_interactions_rev.tex", ///
	replace label b(%9.3f) se(%9.3f) ///
	keep (t_l t_lXhetmargin hetmargin) ///
	order(t_l t_lXhetmargin hetmargin) ///
	scalar(Clusters Mean) sfmt(0 3) ///
	nomtitles ///
	mgroups("HQ Revenues" "Inc Revenues" "Liq Revenues", pattern(1 1 1) prefix(\multicolumn{@span}{c}{) suffix(}) span) ///
	indicate("Month FE = *2mo*""House FE = *house*""Stratum FE = *stratum*") ///
	star(* 0.10 ** 0.05 *** 0.001) ///
	nogaps nonotes compress
