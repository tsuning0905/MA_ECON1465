...
*************
	...
	* Not controlling for neighborhood mean
	...
	* House Quality - Compliance
	cap g hq_y = house_quality_new
	cap g a7_hq_y = a7_house_quality_new 
	eststo: reg hq_y t_l i.house i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & taxes_paid==1, cl(a7)
	...
	* Income - Compliance
	eststo: reg inc_mo_avg t_l i.house i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & taxes_paid==1, cl(a7)
	...
	* Liquidity - Compliance
	eststo: reg econ_ind t_l i.house i.stratum i.time_FE_tdm_2mo_CvL if inlist(tmt,1,2) & taxes_paid==1, cl(a7)
	...
	* House Quality - Compliance
	cap cap g hq_y = house_quality_new
	cap g a7_hq_y = a7_house_quality_new 
	eststo: reg hq_y t_cli i.house i.stratum i.time_FE_tdm_2mo_CvCLI if inlist(tmt,1,3) & taxes_paid==1, cl(a7)
	su hq_y if t_c==1 & time_FE_tdm_2mo_CvCLI!=. & taxes_paid==1
	estadd local Mean=abs(round(`r(mean)',.001))
	estadd scalar Observations = `e(N)'
	estadd scalar Clusters = `e(N_clust)'
	
	* Income - Compliance
	eststo: reg inc_mo_avg t_cli i.house i.stratum i.time_FE_tdm_2mo_CvCLI if inlist(tmt,1,3) & taxes_paid==1, cl(a7)
	su inc_mo_avg if t_c==1 & time_FE_tdm_2mo_CvCLI!=. & taxes_paid==1
	estadd local Mean=abs(round(`r(mean)',.001))
	estadd scalar Observations = `e(N)'
	estadd scalar Clusters = `e(N_clust)'
	
	* Liquidity - Compliance
	eststo: reg econ_ind t_cli i.house i.stratum i.time_FE_tdm_2mo_CvCLI if inlist(tmt,1,3) & taxes_paid==1, cl(a7)
	su econ_ind if t_c==1 & time_FE_tdm_2mo_CvCLI!=. & taxes_paid==1
	estadd local Mean=abs(round(`r(mean)',.001))
	estadd scalar Observations = `e(N)'
	estadd scalar Clusters = `e(N_clust)'

	esttab using "${reploutdir}/incidence_results_nonbhdmean.tex", ///
	replace label b(%9.3f) se(%9.3f) ///
	keep (t_l t_cli) ///
	order(t_l t_cli) ///
	scalar(Clusters Mean) sfmt(0 3) ///
	nomtitles ///
	mgroups("HQ - Paid" "Income - Paid" "Liquidity - Paid" "HQ - Paid" "Income - Paid" "Liquidity - Paid", pattern(1 1 1 1 1 1) prefix(\multicolumn{@span}{c}{) suffix(}) span) ///
